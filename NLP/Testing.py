...
def calc_chunks(h, ref, h_stem, ref_stem, h_syn, ref_syn):
    ...
    while i < len(h):
        ...
        while j < len(ref):
            ...
            if i < len(h) and j < len(ref) and (
                    h[i] == ref[j] or h_stem[i] == ref_stem[j] or check_syns(h_syn[i], ref_syn[j])) and not bitvec[j]:
                while i < len(h) and j < len(ref) and (
                        h[i] == ref[j] or h_stem[i] == ref_stem[j] or check_syns(h_syn[i], ref_syn[j])) and not bitvec[
                    j]:
...
def main():
    ...
    for h1, h2, ref in islice(sentences(), opts.num_sentences):
        ...
        # LEPOR:
        lp1 = math.exp(1 - (len(h1) / float(len(ref)))) if len(h1) > len(ref) else 1 if len(h1) == len(
            ref) else math.exp(1 - (len(ref) / float(len(h1))))
        lp2 = math.exp(1 - (len(h2) / float(len(ref)))) if len(h2) > len(ref) else 1 if len(h1) == len(
            ref) else math.exp(1 - (len(ref) / float(len(h2))))
...
def features(h, ref, met_score, lepor_pen):
    ...
    features_vec = [met_score, lepor_pen, calc_4grams(h, ref)[0], calc_4grams(h, ref)[1], calc_trigrams(h, ref)[0],
                    calc_trigrams(h, ref)[1], calc_bigrams(h, ref)[0], calc_bigrams(h, ref)[
                        1]]  # , calc_4grams(h_pos, ref_pos)[0], calc_4grams(h_pos, ref_pos)[1], calc_trigrams(h_pos, ref_pos)[0], calc_trigrams(h_pos, ref_pos)[1], calc_bigrams(h_pos, ref_pos)[0], calc_bigrams(h_pos, ref_pos)[1]]
